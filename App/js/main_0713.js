...
/** 檢查姓名是否有輸入 */
// const first_name = document.getElementById('my-first-name');
// const last_name = document.getElementById('last-name');

// function nameFunc() {
//     if (first_name.value == '') {
//         //確認name是否為空值，如果為空值，有讓格子顯示警示(增加class=is-invalid)
//         first_name.classList.add('is-invalid');
//     } else if (first_name.value) { //如果name不為空值
//         first_name.classList.remove('is-invalid'); //移除class=is-invalid
//         first_name.classList.add('is-valid')  //增加class=is-valid
//     }

//     if (last_name.value == '') {
//         //確認lastName是否為空值，如果為空值，有讓格子顯示警示(增加class=is-invalid)
//         last_name.classList.add('is-invalid');
//     } else if (last_name.value) { //如果lastName不為空值
//         last_name.classList.remove('is-invalid'); //移除class=is-invalid
//         last_name.classList.add('is-valid');  //增加class=is-valid
//     }
// }
...
/** 手機條件 */
// const telLen = document.getElementById('mobile0');
// const telMsg = document.getElementById('message');
// function telFunc() {
...
const emailArea = document.getElementById('email-id');
function emailFunc() {
    const strEmail = document.getElementById('email-id').value;
...
//區域號碼 (用dropdown menu才需要以下這段，可行在 main.html) 
...
function createParticipantFields(numParticipants) {
    ...
    for (let i = 1; i <= numParticipants; i++) {
        ...
        fieldset.classList.add("border", "p-3", "mb-3", "bg-light");

        const legend = document.createElement("legend");
        legend.classList.add("fs-5");
        legend.textContent = "同伴 " + i;
        fieldset.appendChild(legend);

        // 整個姓名的row
        const fullNameGroup = document.createElement("div");
        fullNameGroup.classList.add("row", "mb-3");
        fieldset.appendChild(fullNameGroup);
        // 姓氏
        const firstNameCol = document.createElement("div");
        firstNameCol.classList.add("col-md-6");
        fullNameGroup.appendChild(firstNameCol);

        const firstNameLabel = document.createElement("label");
        firstNameLabel.textContent = "姓氏";
        firstNameCol.appendChild(firstNameLabel);

        const firstNameInput = document.createElement("input");
        firstNameInput.type = "text";
        firstNameInput.classList.add("form-control");
        firstNameInput.setAttribute('name', 'firstName');  // 增加的 name 屬性
        firstNameInput.setAttribute('id', `firstName${i}`);
        firstNameInput.setAttribute('oninput', `firstNameFunc(${i});`);
        ...
        fullNameGroup.appendChild(lastNameCol);

        const lastNameLabel = document.createElement("label");
        lastNameLabel.textContent = "名字";
        lastNameCol.appendChild(lastNameLabel);

        const lastNameInput = document.createElement("input");
        lastNameInput.type = "text";
        lastNameInput.classList.add("form-control");
        lastNameInput.setAttribute('name', 'lastName');  // 增加的 name 屬性
        lastNameInput.setAttribute('id', `lastName${i}`);
        lastNameInput.setAttribute('oninput', `lastNameFunc(${i});`);
        //lastNameInput.required = true;
        lastNameCol.appendChild(lastNameInput);

        //性別
        const genderGroup = document.createElement("div");
        genderGroup.classList.add("mb-3");
        ...
        genderSelect.setAttribute('name', 'gender'); // 增加的 name 屬性
        genderSelect.required = true;
        genderGroup.appendChild(genderSelect);

        const genderOption1 = document.createElement("option");
        genderOption1.value = "";
        genderOption1.disabled = true;
        genderOption1.selected = true;
        genderOption1.textContent = "選擇性別";
        genderSelect.appendChild(genderOption1);

        const genderOption2 = document.createElement("option");
        genderOption2.value = "male";
        genderOption2.textContent = "男性";
        genderSelect.appendChild(genderOption2);

        const genderOption3 = document.createElement("option");
        genderOption3.value = "female";
        genderOption3.textContent = "女性";
        genderSelect.appendChild(genderOption3);

        //手機
        const contactPhoneGroup = document.createElement("div");
        contactPhoneGroup.classList.add("mb-3");
        contactPhoneGroup.setAttribute('id', 'mobileField');
        fieldset.appendChild(contactPhoneGroup);

        //document.getElementById('mobileField').innerHTML = `<label>聯繫手機</label>`

        const contactPhoneLabel = document.createElement("label");
        contactPhoneLabel.textContent = "聯繫手機";
        contactPhoneGroup.appendChild(contactPhoneLabel);

        const contactPhoneInput = document.createElement("input");
        contactPhoneInput.type = "number";  // tel
        contactPhoneInput.classList.add("form-control");
        contactPhoneInput.setAttribute('name', 'contactPhone'); // 增加的 name 屬性
        contactPhoneInput.setAttribute('id', `mobile${i}`); // 增加的 id 屬性
        contactPhoneInput.setAttribute('oninput', `telFunc(${i});`);
        //contactEmailInput.setAttribute('maxlength', '10');
        ...
        contactPhoneH6.classList.add("ms-2");
        contactPhoneH6.style.display = "inline";
        contactPhoneH6.setAttribute('id', `message${i}`); // 增加的 id 屬性
        contactPhoneGroup.appendChild(contactPhoneH6);


        const copyPhoneBtn = document.createElement("button");
        copyPhoneBtn.type = "button";
        copyPhoneBtn.classList.add("btn", "btn-secondary", "ms-2");
        copyPhoneBtn.textContent = "同上";
        copyPhoneBtn.addEventListener("click", function () {
            const mainContactPhone = document.querySelector("#mobile0").value;  // 取主要參與者的手機
            let currentContactPhoneInput = this.previousElementSibling;  // 
            currentContactPhoneInput.value = mainContactPhone;
        });
        contactPhoneGroup.appendChild(copyPhoneBtn);

        // 信箱
        const contactEmailGroup = document.createElement("div");
        contactEmailGroup.classList.add("mb-3");
        fieldset.appendChild(contactEmailGroup);

        const contactEmailLabel = document.createElement("label");
        contactEmailLabel.textContent = "聯繫電子郵件";
        contactEmailGroup.appendChild(contactEmailLabel);

        const contactEmailInput = document.createElement("input");
        contactEmailInput.type = "email";
        contactEmailInput.classList.add("form-control");
        // contactEmailInput.name = "contactEmail"; // 修改此處的 name 屬性
        contactEmailInput.setAttribute('name', 'contactEmail'); // 增加的 name 屬性
        ...
        copyEmailBtn.classList.add("btn", "btn-secondary", "ms-2");
        copyEmailBtn.textContent = "同上";
        copyEmailBtn.addEventListener("click", function () {
            let mainContactEmail = document.querySelector("#email-id").value;
            const currentContactEmailInput = this.previousElementSibling;
            currentContactEmailInput.value = mainContactEmail;
        });
        contactEmailGroup.appendChild(copyEmailBtn);

        participantFields.appendChild(fieldset);
    }
}
...
document.getElementById("participantForm").addEventListener("submit", function (event) {
    ...
    /*if (selectedAreaCode !== "") {
        let allPhoneNum = selectedAreaCode + phoneNum;
    } else {
        alert("請選擇區域號碼");
    }*/
    let allPhoneNum = selectedAreaCode + phoneNum; // 區域號碼(regionNum)+電話號碼

    /** 手機號碼檢查: 第一個數字有輸入0的話要自動刪除 */
    let telephone = document.getElementById('mobile0').value; //使用 let 設定變數是因為 telephone的值在後面會做更動
    // console.log("原本的手機號碼：" + telephone);
    // 檢查第一個數字是否為0
    ...
    phoneFunc();
    //console.log(Number(totalparticipants) + 1);
    ...
    participantFieldsets.forEach(function (fieldset) {
        ...
        participant.contactPhone = fieldset.querySelector("input[name='contactPhone']").value;
